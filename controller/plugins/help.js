...
import { kodiereAktion, dekodiereAktion, schliesseMenue, pruefeAdmin } from '../lib/manialinkMenu.js'

const AKTION_PRAEFIX = 'help';

export class HelpCommand {
    name           = 'Hilfe'
    author         = 'Greep'
    description    = 'Listet alle Befehle und geladenen Erweiterungen auf'
    ...
    constructor(nextcontrol) {
        ...
        nextcontrol.registerChatCommand(new Classes.ChatCommand(
            ...
            // command description:
            'Zeigt die Liste geladener Erweiterungen an',
        ...
        nextcontrol.registerMenuEintrag(new Classes.MenuEintrag(
            'Allgemein', 'Erweiterungen', kodiereAktion(AKTION_PRAEFIX, 'erweiterungen'), { pluginName: this.name }
        ));
        nextcontrol.registerMenuEintrag(new Classes.MenuEintrag(
            'Admin · Server', 'Admin-Hilfe (Befehlsübersicht)', kodiereAktion(AKTION_PRAEFIX, 'adminhilfe'), { adminOnly: true, pluginName: this.name }
        ));
    ...
    /**
     * Function run, when a player clicks a Hilfe-Menue-Knopf.
    ...
    async onManialinkPageAnswer(params) {
        ...
        if (teile[0] === 'hilfe') await this.commandHelp(params.login, []);
        else if (teile[0] === 'erweiterungen') await this.commandPlugins(params.login, []);
        else if (teile[0] === 'adminhilfe') {
            if (!pruefeAdmin(this.nextcontrol, params.login)) return;
            await this.commandAdminHelp(params.login, []);
        }
     ...
     */
     async commandPlugins(login, params) {
        ...
        this.nextcontrol.client.query('ChatSendServerMessageToLogin', ['\n$8F0---------- Geladene Erweiterungen ----------$z$s\n' + listArray.join('\n'), login]);
    }
    ...
    async commandAdminHelp(login, params) {
        ...
        this.nextcontrol.client.query('ChatSendServerMessageToLogin', ['\n$A80---------- Admin-Befehlsübersicht ----------$z$s\n' + listArray.join('\n'), login]);
    }
