...
export class ListsPlugin {
     ...
     * Plugin description
     */
    description = 'Ermöglicht Spielern, Karten oder Spieler abzufragen und für spätere Befehle zu speichern';
     ...
     */
    constructor(nc) {
        nc.registerChatCommand(new Classes.ChatCommand('liste', this.listCommand, 'Listet Karten oder Spieler zur späteren Verwendung auf!', this.name));
    ...
    async toChat(login, list, pageNr) {
        ...
        if (list === cmaps) {
            ...
            let items = this.nextcontrol.lists.maps.get(login),
                ...
                message = util.format(Sentences.lists.header, {type: 'Karten', pg: pageNr, pages: Math.ceil(items.length / ITEMS)});
        ...
        if (list === cplayer) {
            ...
            let items = this.nextcontrol.lists.players.get(login),
                lowerBound = (pageNr - 1) * ITEMS,
                upperBound = pageNr * ITEMS,
                message = util.format(Sentences.lists.header, {type: 'Spieler', pg: pageNr, pages: Math.ceil(items.length / ITEMS)});
