...
export class SamplePlugin {
     ...
     * Plugin description
     */
    description    = 'Ausführlich dokumentiertes Beispiel-Plugin, um direkt in die Plugin-Entwicklung einzusteigen'
    ...
    constructor(nextcontrol) {
        ...
        nextcontrol.registerChatCommand(new Classes.ChatCommand(
            ...
            // command description:
            'Einfaches Hallo-Welt-Kommando, zur Demonstration der Befehlsumsetzung',
        ...
        nextcontrol.registerAdminCommand(new Classes.ChatCommand(
            ...
            // description:
            'Hallo Welt, aber für Admins.',

            this.name
        ));

        // if this plugin requires a collection, that NextControl doesn't provide by itself (e. g. one you need for keeping map of custom statistics or so)
    ...
    async commandHelloWorld(login, params) {
        ...
        this.nextcontrol.client.query('ChatSendServerMessageToLogin', ['$f00Hallo Welt!', login]);
    }
    ...
    async commandAdminHelloWorld(login, params) {
        ...
        this.nextcontrol.client.query('ChatSendServerMessageToLogin', ['$f0fHallo Welt!', login]);
    }
