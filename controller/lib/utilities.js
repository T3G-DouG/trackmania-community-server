 ...
 * @param {string} status Status code, su = startup, dg = debug, r = running, db = database, w = warning, er = error, l = log, i = importamt, ok = success
...
export function logger(status, text) {
    ...
    if (status === 'ok') console.log( chalk.bold(chalk.green('OK: ')) + text + ' (' + runtime + ')' );
...
/**
 * Escapt XML-Sonderzeichen in dynamischen Werten (z.B. Spielernamen), bevor sie
 * in ManiaLink-XML eingesetzt werden. TM-Formatierungscodes ($f00 etc.) bleiben
 * unveraendert, da ManiaLink sie wie im Chat interpretiert.
 * @param {string} text
 */
