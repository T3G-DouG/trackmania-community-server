 ...
 * Aktion, siehe controller/lib/manialinkMenu.js).
 */
export class MenuEintrag {
    constructor(kategorie, label, aktion, { adminOnly = false, pluginName } = {}) {
...
export class Status {
     ...
     */
    getPlayer(login) {
        // create temporary empty object, should we not find a player in the list
        // this should only happen if the player joined when the script was offline.
        let player = {
            name: "",
            login: ""
        };
...
export class ModeSettingsController {
    ...
    async init() {
        this.defaultSettings = await this.nextcontrol.client.query('GetModeScriptSettings');
     ...
     * @returns {Promise<void>}
     */
    async readMatchSettings() {
        // reload file
        await this.nextcontrol.client.query('LoadMatchSettings', [this.nextcontrol.status.directories.maps + '/MatchSettings/' + Settings.trackmania.matchsettings_file]);
    ...
    /**
     * Keeps the changes done during the current map
     */
    keepTempSettings() {
     ...
     */
    hasSetting(name) {
        return Object.keys(this.defaultSettings).includes(name);
    }
}