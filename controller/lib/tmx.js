...
export class TMX {
    ...
    static async getID(uid) {
        ...
        let response;
        try {
            response = await got(url, {responseType: 'json', headers: this.headers});
        } catch (error) {
            // TMX nicht erreichbar/Fehler -- darf den Controller nicht abschiessen (kein
            // globaler unhandledRejection-Handler), Aufrufer behandeln -1 ohnehin als
            // "nicht auf TMX verfuegbar".
            logger('er', `TMX.getID fehlgeschlagen fuer ${uid}: ${error.message}`);
            return -1;
        }
    ...
    /**
     * Holt Map-Infos von der TMX-REST-API per Map-UID (fuer die Karma-Voting-Kandidaten-
     * bewertung, siehe scripts/lib/tmxCache.js). Gleicher Feld-Satz + gleiches Mapping wie
     * website/api.php::tmxMapInfoAbrufen() (derselbe Cache "tmxMapInfo" wird von beiden
     * Systemen genutzt) -- Authors[].User.Name ist verschachtelt, nicht Authors[].Name direkt.
     * Wirft NIE -- liefert bei jedem Fehler {gefunden:false, fehler:true, daten:null}.
     * @param {String} uid Map-UID
     * @returns {Promise<{gefunden: Boolean, fehler: Boolean, daten: Object|null}>}
     */
    static async getMapInfoByUid(uid) {
        const felder = 'MapId,Name,GbxMapName,Authors,Tags,Medals.Author,AwardCount,Length,Difficulty,UploadedAt,UpdatedAt,Images';
        const url = `${this.site}/api/maps?uid=${encodeURIComponent(uid)}&fields=${encodeURIComponent(felder)}`;

        let response;
        try {
            response = await got(url, {
                headers: { ...this.headers, 'User-Agent': 'NextControl-Controller/1.0 (privater LAN-Server)' },
                responseType: 'json',
                timeout: { request: 5000 },
                retry: { limit: 0 }, // Negativ-Cache in tmxCache.js uebernimmt die Wiederholungslogik, nicht got
            });
        } catch (error) {
            return { gefunden: false, fehler: true, daten: null };
        }

        const treffer = response.body?.Results?.[0];
        if (!treffer) return { gefunden: false, fehler: false, daten: null };

        const autoren = [];
        for (const a of treffer.Authors ?? []) {
            const name = a?.User?.Name;
            if (name) autoren.push(String(name));
        }
        const tags = [];
        for (const t of treffer.Tags ?? []) {
            const farbe = t?.Color;
            tags.push({
                name: String(t?.Name ?? ''),
                farbe: (typeof farbe === 'string' && /^[0-9a-fA-F]{6}$/.test(farbe)) ? farbe : null,
            });
        }

        const daten = {
            mapId: Number(treffer.MapId ?? 0),
            name: String(treffer.Name ?? treffer.GbxMapName ?? ''),
            autoren,
            tags,
            schwierigkeit: treffer.Difficulty != null ? Number(treffer.Difficulty) : null,
            laenge: treffer.Length != null ? Number(treffer.Length) : null,
            autorenzeitTmx: treffer.Medals?.Author != null ? Number(treffer.Medals.Author) : null,
            awards: treffer.AwardCount != null ? Number(treffer.AwardCount) : null,
            hochgeladenAm: treffer.UploadedAt ?? null,
            hatBild: Boolean(treffer.Images?.length),
        };

        return { gefunden: true, fehler: false, daten };
    }

    /**
     * Holt die vollen TMX-Daten einer Map und schreibt sie sofort in den Cache
     * "tmxMapInfo" (gleiches Schema wie scripts/lib/tmxCache.js/website/api.php) --
     * fuer die Ersterfassung neuer Maps (sicherstelleMapInDb(), addmapToServer(),
     * Dashboard-TMX-Download), damit die TMX-Signale ohne TTL-Warteschleife sofort
     * fuers Karma-Voting-Scoring verfuegbar sind. Als fire-and-forget gedacht (nicht
     * awaiten) -- faengt daher alle Fehler selbst ab und wirft NIE.
     * @param {import('mongodb').Db} mongoDb
     * @param {String} uid Map-UID
     * @returns {Promise<void>}
     */
    static async aktualisiereCache(mongoDb, uid) {
        try {
            const ergebnis = await this.getMapInfoByUid(uid);
            await mongoDb.collection('tmxMapInfo').updateOne(
                { _id: uid },
                { $set: { geladenAm: new Date(), gefunden: ergebnis.gefunden, fehler: ergebnis.fehler, daten: ergebnis.daten } },
                { upsert: true }
            );
        } catch (error) {
            logger('er', `TMX.aktualisiereCache fehlgeschlagen fuer ${uid}: ${error.message}`);
        }
    }
